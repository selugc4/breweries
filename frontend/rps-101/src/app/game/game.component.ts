...
export class GameComponent implements OnInit {
  ...
  constructor(private randomDeckService: RandomDeckService, private cardApiService: CardApiService, private appComp: AppComponent ){}
  ...
  startMatch(){
    ...
    if(this.deck === undefined){
      this.playerDeck = this.getPlayerDeck(this.randomDeckService.getRandomDeck().cards);
    }else{
      this.playerDeck= this.getPlayerDeck(this.deck.cards);
    }

    this.enemyDeck = this.getPlayerDeck(this.randomDeckService.getRandomDeck().cards);
    this.startTimer();
  }
 ...
 startTimer() {
    ...
    this.interval = setInterval(() => {
      ...
      if(this.roundTime <= 0){
        this.rounds[this.round] = this.playRound(Math.floor(Math.random()*(5-this.round)));
        this.nextRound();
      }
  ...
  pauseTimer() {
    ...
    reloadCurrentPage() {
     ...
     apiCall(chosenCard: string, enemyChosenCard: string): string[] {
      var played = this.cardApiService.getMatchResult(chosenCard, enemyChosenCard);
