...
export class GameComponent implements OnInit {
    ...
    async playCard(cardIndex: number) {
        ...
        this.indexEnemyCard = Math.floor(
            Math.random() * (5 - this.currentRound)
        );
    ...
    throwCards(userId: number, oponentId: number) {
        ...
        const oponentCard = document.querySelector(
            '#oponent-card-' + oponentId
        );

        this.flippedEnemyCards[this.indexEnemyCard] = false;
    ...
    removePlayedCards() {
        ...
        this.flippedEnemyCards.splice(this.indexEnemyCard, 1);
    ...
    startGame() {
        ...
        switch (this.gamemode) {
            case GameMode.Exposed:
                this.flippedEnemyCards = Array(5).fill(false);
                break;
            case GameMode.Hidden:
                this.flippedEnemyCards = Array(5).fill(true);
                break;
            case GameMode.Rivals:
                this.flippedEnemyCards = [false, false, true, true, true];
        }
    ...
    startTimer() {
        ...
        this.timerInterval = setInterval(() => {
            if (!this.isPausedTimer) {
                ...
                this.spinnerValue =
                    this.currentRoundTime / (this.ROUND_TIME / 100);
    ...
    nextRound() {
        if (this.currentRound >= 4) {
            ...
            if (this.gamemode == GameMode.Rivals) {
                this.flippedEnemyCards[0] = false;
                if (this.flippedEnemyCards.length > 1) {
                    this.flippedEnemyCards[1] = false;
                }
            }
